...
## 2026-07-02  -  Model packaging: dual-format committed artifacts
...
## 2026-07-04  -  API input validation: strict at the boundary
...
## 2026-07-11  -  Deployment platform: local Kubernetes instead of EKS
**Decision:** Deploy to Docker Desktop's built-in Kubernetes cluster (2 nodes, v1.36) with plain manifests  -  namespace, 2-replica Deployment with /health readiness+liveness probes, LoadBalancer Service on port 80. The image is consumed from the local store (imagePullPolicy: IfNotPresent); no registry.
**Rationale:** The available AWS account cannot create IAM roles and cannot expose public services (private VPC/subnets only). eksctl provisions IAM roles via CloudFormation, so EKS cluster creation fails under those guardrails. The assignment FAQ explicitly accepts local Kubernetes (Minikube, Docker Desktop) for the deployment task, and the deliverables accept local access instructions in place of a public URL. Local K8s satisfies the rubric fully  -  deployment manifest + service YAML, LoadBalancer exposure, verifiable endpoints  -  with zero account risk and locally reproducible verification. Kubernetes ignores the Dockerfile HEALTHCHECK, so the probes point at /health; with the model loaded at import, a pod with a broken artifact never becomes Ready.
**Alternatives considered:** EKS with pre-existing IAM roles + internal ELB (feasibility unverifiable in the restricted account; every step relayed through a SageMaker terminal); ECS in private subnets (works in the account, but the FAQ expects Kubernetes Deployment/Service YAML  -  an ECS task definition argues with the rubric rather than satisfying it).

## 2026-07-11  -  Monitoring: instrumented API + in-cluster Prometheus/Grafana
**Decision:** The API exposes /metrics via prometheus-fastapi-instrumentator (standard HTTP metrics) plus a custom heart_disease_predictions_total{risk_label} counter. Prometheus (static scrape of the service DNS, 15s) and Grafana (datasource + dashboard fully provisioned from ConfigMaps, anonymous viewer) run in the same namespace behind LoadBalancers on 9090/3000. The stack is ephemeral  -  no persistent volumes; metrics reset with pods, which is acceptable for a demo and documented.
**Rationale:** The assignment asks for request logging (already present as structured log lines) plus simple monitoring; a real Prometheus+Grafana path demonstrates the production pattern end-to-end, and full provisioning makes it reproducible with one kubectl apply. The domain counter shows ML-aware monitoring (class-mix drift is visible), not just HTTP plumbing.
**Alternatives considered:** Lightweight JSON /metrics endpoint + log screenshots (sufficient per FAQ but shows no real monitoring integration); kube-prometheus-stack via Helm (heavyweight, needs Helm which this project deliberately avoids).

## 2026-07-12  -  Monitoring: per-pod scrape via headless Service
**Decision:** Prometheus discovers API pods through a dedicated headless Service (`heart-disease-api-headless`, clusterIP: None) with `dns_sd_configs` (A records, port 8000), replacing the static scrape of the load-balancing Service.
**Rationale:** Scraping through the LoadBalancer alternated pods per scrape, so counter series mixed two pods' values; Prometheus treated downward jumps as counter resets and inflated rate() far beyond real traffic (observed ~263 apparent non-2xx in 30 minutes against a true total of 34 since pod start). Per-pod targets keep counters monotonic and make sum() across instances correct.
**Alternatives considered:** Accepting the artifact with a documented explanation (undermines the dashboard's credibility); kube-prometheus-stack pod discovery via the Kubernetes API (needs RBAC and Helm, both deliberately avoided).

## 2026-07-12  -  Standalone inference notebook
**Decision:** Add `notebooks/03_inference.ipynb` (jupytext-paired like 01/02, executed outputs committed): loads the committed joblib artifact directly with no API server or MLflow store, prints the exported model's provenance from `model_metadata.json`, scores `api/sample_request.json` (prediction 0, P(disease)≈0.33  -  matching the API's response for the same record), and runs batch inference over the full cleaned CSV.
**Rationale:** The deliverables list names three notebook/script roles  -  EDA, training, inference. Inference so far existed only embedded elsewhere (the API endpoint, the demo server, `tests/test_export.py`); a grader working down the checklist finds nothing labelled inference. The notebook also doubles as the FAQ's "preprocessing pipelines must be reusable during inference" proof, since it feeds raw columns to the artifact with no API in the path.
**Alternatives considered:** A CLI script (`ml/models/infer.py`)  -  works, but a notebook shows the outputs inline, which is what gets read; pointing at test_export.py as the inference evidence  -  tests are verification, not a presentation artifact.
